                ...
                <div className="relative">
                    {session.user.image ? (
                        <Image
                            ...
                            width={44}
                            height={44}
                            className="rounded-full ring-2 ring-slate-700 group-hover:ring-cyan-500/50 transition-all"
                        />
                    ) : (
                        <div className="w-11 h-11 rounded-full bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white font-semibold text-lg group-hover:ring-2 group-hover:ring-cyan-500/50 transition-all">
                            {session.user.name?.[0]?.toUpperCase() || session.user.email?.[0]?.toUpperCase() || "U"}
                        </div>
                    )}
                    
                    {/* Online Badge - petit point vert comme sur les réseaux sociaux */}
                    <span className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 border-2 border-slate-900 rounded-full"></span>
                    
                    {/* Admin Badge (si admin) */}
                    {session.user.isAdmin && (
                        <span className="absolute -top-1 -right-1 w-5 h-5 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg">
