...
from config import *
from data import MATERIALS_DB
from utils import get_monospace_font, get_ui_font, get_linux_desktop_scale
from ui.tabs import summary as summary_tab
...
# Linux Scaling Hack (simplified)
try:
    if platform.system() == "Linux":
        scale, method = get_linux_desktop_scale()
...
# Check for Safe Mode
SAFE_MODE = os.environ.get("ROCKET_SAFE_MODE") == "1"

# Essayer d'importer RocketCEA
if SAFE_MODE:
    print("🛡️ RocketCEA disabled by Safe Mode")
    HAS_ROCKETCEA = False
    CEA_Obj = None
else:
    try:
...
from ui.components import MultiRowTabview
...
class RocketApp:
    ...
    # update_thermal_graphs moved to ui/tabs/thermal.py
    ...
    def init_summary_tab(self):
        ...
        summary_tab.init(self)

    # insert_colored_summary moved to ui/tabs/summary.py
    ...
    def init_thermal_tab(self):
        """Onglet Thermique - Profils et échanges"""
        thermal_tab.init(self)
    ...
    def update_cad_preview(self):
        ...
        cad_tab.update_cad_preview(self)

    def export_dxf(self):
        """Exporte le DXF (via le bouton de la toolbar principale)."""
        cad_tab.export_dxf(self)
    ...
    def init_heatmap_tab(self):
        ...
        heatmap_tab.init(self)

    def update_heatmap(self):
        """Met à jour la carte thermique 2D"""
        heatmap_tab.update(self)
        ...
        try:
            ...
            if use_fallback:
                ...
                if os.path.exists("cea_bridge.py"):
                    try:
                        import subprocess
                        print("🚀 Tentative d'utilisation de cea_bridge.py...")
                        
                        cmd = ["py", "-3.10", "cea_bridge.py", 
                               "--ox", str(ox), "--fuel", str(fuel), 
                               "--pc", str(pc), "--pe", str(pe_des), 
                               "--mr", str(mr), "--pamb", str(pamb)]
                        ...
                        if result.returncode == 0:
                            ...
                            if data.get("status") == "success":
                                ...
                                eps = res["eps"]
                                cstar_mps = res["cstar_m_s"]
                                isp_vac = res["isp_vac_s"]
                                isp_amb = res["isp_amb_s"]
                                ...
                                print("✅ Bridge CEA succès!")
                                bridge_success = True
                            else:
                                print(f"❌ Erreur Bridge: {data.get('message')}")
                        else:
                             print(f"❌ Echec appel Bridge. Return code: {result.returncode}")
                             print(f"Stderr: {result.stderr}")

                    except Exception as e:
                        print(f"❌ Exception Bridge: {e}")

                if not bridge_success:
                    # --- FALLBACK ULTIME (Gaz Parfait) ---
                    print("⚠️ Mode Fallback (Gaz Parfait)")
                    gamma = 1.2
                    molwt = 24.0
                    R_univ = 8314.46
                    R_gas = R_univ / molwt
            ...
            # Common variable names for later usage
            MW = molwt
...
"""
            ...
            summary_tab.insert_colored_summary(self, summary, cooling_status, coolant_warning)
